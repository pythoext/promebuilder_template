<%
  ...
  def get_version():
    with open('version') as ov:
      return ov.read().strip()
%>
...
<%def name="show_module(module)">
  ...
  <h1 class="title">${'Namespace' if module.is_namespace else  \
                      ...
                      version ${get_version() if module.is_package and not module.supermodule else ''}</h1>
  </header>
...
<%def name="module_index(module)">
  ...
  <nav id="sidebar">
    ...
    <h1>Promlib-transactional Documentation</h1>
...
%>
  ...
  % if syntax_highlighting:
    <link rel="stylesheet preload" as="style" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.18.1/styles/${hljs_style}.min.css" crossorigin>
  %endif
